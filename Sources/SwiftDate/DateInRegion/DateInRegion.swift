...
public struct DateInRegion: DateRepresentable, Decodable, Encodable, CustomStringConvertible, Comparable, Hashable {
	...
	/// Formatter used to transform this object in a string. 
    ///
    /// By default is `nil` because SwiftDate uses the thread shared formatter in order
    /// to avoid expensive init of the `DateFormatter` object. However, if you need of a
    /// custom behavior you can set a valid value.
    ///
	...
	/// If you don't know the format leave it `nil` and parse is done between all formats in ``DateFormats/autoFormats``
	/// and the ordered list you can provide in ``SwiftDate/SwiftDate/autoFormats``
    /// (with attempt priority set on your list).
	///
	/// - Parameters:
    ...
    public init?(_ string: String, format: String? = nil, region: Region = SwiftDate.defaultRegion) {
        guard let date = DateFormats.parse(
            string: string,
            format: format,
            region: region
        ) else {
	...
	/// Date is therefore expressed in passed region (`DateComponents`'s `timezone`,`calendar` and `locale` are ignored
	/// and overwritten by the region if not `nil`).
	///
	/// - Parameters:
	///   - configuration: configuration callback
	///   - region: region in which the date is expressed.
	///				Ignore to use ``SwiftDate/SwiftDate/defaultRegion``, `nil` to use `DateComponents` data.
    ///
